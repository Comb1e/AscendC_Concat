...
`1d9435c` 增加非对齐 FP16/FP32 小行 staging + Gather 分支，官方结果为：

| Case | `2c97dca` 历史最好/us | `1d9435c`/us | 差值/us | 结论 |
| --- | ---: | ---: | ---: | --- |
| 1 | 11.140 | Run failed | - | 功能失败，无性能值 |
| 2 | 32.4705 | 34.656 | +2.1855 | +6.73% |
| 3 | 22.390 | 22.580 | +0.190 | +0.85% |
| 4 | 103.653 | 109.076 | +5.423 | +5.23% |
| 5 | 403.9585 | 414.360 | +10.4015 | +2.57% |

Case1 失败后不能计算官方 `prof_sum`；其余四项小计为 `580.672 us`，比历史最好相同四项的
`562.472 us` 慢 `18.200 us`（`3.24%`）。隐藏 shape 与实际 tiling key 不可见，因此不能仅凭
Case2 至 Case5 通过就断言它们都走了回退路径；但 Case1 已足以否定原提交。
...
官方系统对 `763fc9a` 的五项精度均通过，结果如下。对照项使用修改前的官方历史最好
`2c97dca=573.612 us`；测试与文档提交不改变算子源码。

| Case | 深度 2 基线/us | 深度 1/us | 变化/us | 变化率 |
| --- | ---: | ---: | ---: | ---: |
| Case1 | 11.140 | 10.9805 | -0.1595 | -1.43% |
| Case2 | 32.4705 | 33.4305 | +0.9600 | +2.96% |
| Case3 | 22.390 | 21.9400 | -0.4500 | -2.01% |
| Case4 | 103.653 | 100.4120 | -3.2410 | -3.13% |
| Case5 | 403.9585 | 406.6185 | +2.6600 | +0.66% |
| 合计 | 573.612 | 573.3815 | -0.2305 | -0.040% |

`573.3815 us` 是当前新的官方最低结果，但总收益只有 `0.2305 us`，明显小于各 Case 之间相反
方向的波动。Case1/3/4 合计改善 `3.8505 us`，被 Case2/5 的 `3.6200 us` 回退几乎抵消。
因此该结果支持将逻辑深度 1 保留为新基线，不能仅凭总分断言它稳定改善了 MTE2/MTE3 流水；
每核 1/2/8 迭代的本地 A/B 仍是区分固定队列开销与稳态 overlap 的必要证据。

下一轮不继续叠加 Buffer 或改变 tile。CANN 8.5 内置 Concat 对等形输入使用专门模板，直接
复用统一的拼接片段长度；当前项目只有 `inputCount <= 16` 能避开 `GetDesc`，17～256 个等形
输入仍由每个活跃 AIV 重复复制描述符并遍历 shape。下一项实验只在 `inputCount > 16` 且 Host
确认所有片段等长时传入统一 `segmentBytes`，Kernel 改用 `GetDataPtr`；非等形输入和 16 输入
以内保持 `763fc9a` 数据路径，以较小 Tiling 增量换取长等形列表可能显著降低 Scalar 开销。
...
全局 `ascendc-operator-tuning` skill 同步增加了上述队列参数区分、纯搬运两阶段判断、ELF
`.text` 对照方法和每核 1/2/多迭代 A/B 设计，并已通过 `skill-creator` 校验。
...
| `compact_offset_aligned` | 16 个非对齐片段，但所有 offset 段起点落在 8 个索引的 Vector 对齐边界 |
| `compact_output_tail` | 输出行 514B，验证多行写回按 32B 自动推进 UB、GM 保持紧密连续 |
| `compact_batch_one` | 每核恰好一行，隔离 `blockCount=1` 与多行 stride 行为 |
